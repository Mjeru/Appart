		...
		$(`.all-filters`).on(`click`, function (e) {
			if (country.childNodes.length == 0) {
				...
				$.ajax({
					...
					context: document.body,"use strict";
					...
					$(document).ready(function () {
					  ...
					  $(".tostyle").each(function (index) {
						...
						[].forEach.call(document.querySelectorAll(".whith-clear"), function (element) {
						  element.oninput = function (e) {
							if (e.target.value !== "") {
							  e.target.parentNode.querySelector(".clear-input").style = "opacity: 1;";
							} else {
							  e.target.parentNode.querySelector(".clear-input").style = "opacity: 0;";
							}
						  };
						});
					  ...
					  [].forEach.call(document.querySelectorAll(".clear-input"), function (element) {
					  ...
					  region.onchange = function (e) {
						...
						if (!street.classList.contains("popup-disabled-select")) street.classList.add("popup-disabled-select");
						if (!district.classList.contains("popup-disabled-select")) district.classList.add("popup-disabled-select");
						city.appendChild(blunk.cloneNode());
					  ...
					  district.onchange = function (e) {
						...
						street.appendChild(blunk.cloneNode());
						$.ajax({
						  url: "".concat(urlGeo, "?type=street&country=").concat(country.value, "&region=").concat(region.value, "&city=").concat(value),
						  context: document.body,
						  success: function success(html) {
							html.list.forEach(function (element) {
							  var newLi = document.createElement("option");
					  ...
					  document.querySelector(".changeLang").onclick = function (e) {
						...
						var url = "country=".concat(lang, "&area=").concat(metr, "&currency=").concat(val);
						...
						if (curentUrl.match(/country\=[A-z]+\&area\=[A-z]+\&currency\=[A-z]+/)) {
						  ...
						  curentUrl = curentUrl.replace("".concat(mainUrl), "".concat(mainUrl, "&").concat(url));
						  location.replace(curentUrl);
						}
					  };
					
					  [].forEach.call(document.querySelector(".nav-mobile").children[0].children, function (el) {
